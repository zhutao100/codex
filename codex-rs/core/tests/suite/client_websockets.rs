...
async fn responses_websocket_streams_request() {
    ...
    assert_eq!(
        handshake.header(ORIGINATOR_HEADER),
        Some(codex_core::default_client::originator().value)
    );
    assert_eq!(
...
async fn responses_websocket_v2_creates_without_previous_response_id_when_non_input_fields_change()
{
    ...
    assert_eq!(second["type"].as_str(), Some("response.create"));
    assert_eq!(second["previous_response_id"], serde_json::Value::Null);
    assert_eq!(
        second["input"],
        serde_json::to_value(&prompt_two.input).unwrap()
    );
...
async fn responses_websocket_v2_after_error_uses_full_create_without_previous_response_id() {
    ...
    let err = stream_until_error(&mut session, &harness, &prompt_two).await;
    match err {
        CodexErr::InvalidRequest(message) => {
            assert!(message.contains("synthetic websocket failure"));
        }
        other => panic!("unexpected websocket error: {other:?}"),
    }

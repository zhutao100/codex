...
"#
        ...
        fn diverged_branch_count_uses_merge_base_with_selected_next_tag() {
            ...
            // then tag `rust-v0.98.0` one commit later.
            ...
            write_file(repo, "Cargo.toml", &cargo_toml_with_version("0.98.0"));
            commit_all(repo, "release 0.98.0");
            run_git(repo, &["tag", "rust-v0.98.0"]);

            // Trunk line: create >80 newer tags so `rust-v0.98.0` is outside the default scan window.
            ...
            match scenario {
                Scenario::Diverged {
                    ...
                    assert_eq!(baseline.is_none(), true);
                    assert_eq!(next.name, "rust-v0.98.0");
                    assert_eq!(ahead_from_common, 3);
                }
                other => panic!("expected diverged scenario, got {other:?}"),
            }

            assert_eq!(version, format!("0.98.0-branch.3.{head_short}"));
        }
        ...
        fn diverged_tag_flood_does_not_hide_relevant_stable_release_line() {
            ...
            run_git(repo, &["tag", "rust-v0.98.0-alpha.1"]);

            write_file(repo, "trunk.txt", "alpha.2\n");
            commit_all(repo, "alpha.2");
            run_git(repo, &["tag", "rust-v0.98.0-alpha.2"]);

            // Flood later tags so the stable `rust-v0.98.0` tag is outside the default scan window.
            for i in 1..=81 {

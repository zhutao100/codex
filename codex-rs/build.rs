...
fn main() {
    ...
    let head_short = git_trim(&["rev-parse", "--short=8", "HEAD"]).unwrap_or_else(|| "unknown".to_string());
...
fn newest_common_commit_and_next_tag(all: &[TagInfo], max_tags: usize) -> Option<(String, TagInfo)> {
    // Heuristic: scan newest *release lines* first (distinct core versions), pick merge-base closest
    // to HEAD (min distance). This avoids large prerelease tag volumes pushing the relevant stable
    // tag outside the scan window.
    let mut best: Option<(u64, String)> = None;
    let mut seen_cores: HashSet<(u64, u64, u64)> = HashSet::new();
    let mut cores_scanned = 0usize;
    for tag in all.iter().rev() {
        let core_key = (tag.version.major, tag.version.minor, tag.version.patch);
        if !seen_cores.insert(core_key) {
            continue;
        }
        cores_scanned += 1;
        if cores_scanned > max_tags {
            break;
        }
    ...
    // IMPORTANT:
    // `common_commit` above is computed against whichever tag produced the closest merge-base within the scan window.
    // The selected `next` tag can live on a different release line that shares a *newer* merge-base with HEAD
    // (for example, when a stable release branch diverged from trunk and the scan window is dominated by trunk tags).
    //
    // For "branch.<n>" we want `n` to mean "commits since we diverged from the chosen release line (`next`)".
    // So anchor the common commit to the merge-base between HEAD and the selected `next` tag.
    let common_with_next = git_trim(&["merge-base", "HEAD", &next.name]).unwrap_or(common_commit);
    Some((common_with_next, next))
}
...
fn build_version_string(s: &Scenario, cargo_manifest_version: Option<&str>, head_short: &str, dirty: bool) -> String {
    ...
    let mut v = match s {
        ...
        Scenario::ContainedInNext { baseline, next, behind } => {
            // You are *before* the next release tag commit, so use next_version - pre.<behind>.<sha>.
            let prefix = format!("pre.{behind}.{head_short}");
        ...
        Scenario::PostLatestTag { baseline, ahead } => {
            ...
            let prefix = if baseline.version.pre.is_empty() {
                format!("dev.{ahead}.{head_short}")
            } else {
                format!("{}.dev.{ahead}.{head_short}", baseline.version.pre)
            };
        ...
        Scenario::Diverged {
            ...
            let prefix = format!("branch.{ahead_from_common}.{head_short}");
            let pre = with_baseline_prerelease(baseline.as_ref(), &next.version, &prefix);
            with_prerelease(&next.version, &pre)
        }

        Scenario::Fallback => with_prerelease(&base_fallback, &format!("local.{head_short}")),
    };
...
fn extract_semver_from_tag(tag: &str) -> Option<Version> {
    ...
    for i in 0..bytes.len() {
        let c = bytes[i];
        let starts = c.is_ascii_digit() || (c == b'v' && bytes.get(i + 1).is_some_and(u8::is_ascii_digit));
        if !starts {
            continue;
        }
        ...
        if best.is_none_or(|(bi, bj)| (j - i) > (bj - bi)) {
            best = Some((i, j));
        }
    }
